...
- **Where this lives now.** `tone` and `compact` are fields on the Sanity `client` document (see PRODUCT.md § Content Management), set per logo in the Studio — same judgment call as before, just made by whoever adds the client instead of in code. The Studio field descriptions restate the two rules above; whoever is editing the roster shouldn't need to read this file to get them right.
...
**Not managed in Sanity.** The 2026-09-22 CMS migration (see PRODUCT.md § Content Management) moved copy, contacts, and the client roster into Sanity, but deliberately left this photography and its captions in code. The grading/cropping pipeline above is source-specific and one-off; a "swap the hero image" CMS field would invite an upload that skips the watermark-exclusion crop and the saturation/contrast grade this section describes, breaking the Full-Colour Evidence Rule silently. If the photography ever needs to be genuinely editable, that's a deliberate follow-up (a dedicated image field plus a documented crop/grade contract), not a drive-by field addition.
